...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-05-2020 a las 00:07:52
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LISTAR_COMBO_DEPARTAMENTO` ()  SELECT  * FROM ubdepartamento$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LISTAR_COMBO_DISTRITO` (IN `IDPROVINCIA` INT)  SELECT * FROM ubdistrito where idProv=IDPROVINCIA$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LISTAR_COMBO_PROVINCIA` (IN `IDDEPARTAMENTO` INT)  SELECT * FROM ubprovincia where idDepa=IDDEPARTAMENTO$$
